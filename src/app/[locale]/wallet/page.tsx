...
export default function WalletPage() {
  ...
  const [mobileAccountCurrency, setMobileAccountCurrency] = useState<'LOCAL' | 'USD'>('LOCAL');
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      {/* BOUTON RAPIDE - Envoyer à un ami PayMaestro */}
      <button
      ...
      >
        ...
        <p className="font-bold text-green-800 dark:text-green-300">Envoyer à un ami PayMaestro</p>
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 border-2 border-amber-200 dark:border-amber-800/50">
            <CardContent className="p-6 space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (country) {
                        ...
                        setMobileCurrency(country.code);
                        setMobileOperator(country.operators[0] || 'Orange');
                        setMobileAccountCurrency('LOCAL');
                      }
                    }}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Type de compte</label>
                  <div className="flex gap-2 mt-1">
                    <button
                      type="button"
                      onClick={() => setMobileAccountCurrency('LOCAL')}
                      className={`flex-1 px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                        mobileAccountCurrency === 'LOCAL'
                          ? 'bg-amber-600 text-white shadow-sm'
                          : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'
                      }`}
                    >
                      <img src={`https://flagcdn.com/w40/${mobileCountry.iso2}.png`} alt="" className="w-4 h-3 inline-block mr-1.5 rounded" />
                      {mobileCurrency} (Local)
                    </button>
                    <button
                      type="button"
                      onClick={() => setMobileAccountCurrency('USD')}
                      className={`flex-1 px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                        mobileAccountCurrency === 'USD'
                          ? 'bg-amber-600 text-white shadow-sm'
                          : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'
                      }`}
                    >
                      <img src="https://flagcdn.com/w40/us.png" alt="" className="w-4 h-3 inline-block mr-1.5 rounded" />
                      USD
                    </button>
                  </div>
                </div>
              ...
              <div className="flex gap-4">
                <input
                  ...
                  placeholder={`Montant en ${mobileAccountCurrency === 'USD' ? 'USD' : mobileCurrency}`}
