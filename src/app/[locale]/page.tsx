...
// Taux de change simulés (seront remplacés par l'API backend)
const LIVE_RATES = [
...
export default function HomePage() {
  ...
  const receiveAmount = Math.round(netUSD * rate);
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-7 space-y-6">
          <Badge variant="info" className="px-3 py-1 text-xs">
            ⚡ Retrait PayPal instantané vers l&apos;Afrique
          </Badge>
            ...
            <CardContent className="p-6 sm:p-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-7 pr-12 py-3 border border-slate-200 rounded-xl text-lg font-extrabold text-slate-900 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500"
                    placeholder="100"
                  />
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2.5 text-xs text-slate-500">
                <div className="flex justify-between">
                  <span>{t('calculator.fee')}</span>
                  <span className="font-semibold text-red-500">-${platformFee.toFixed(2)} USD</span>
                </div>
                <div className="flex justify-between">
                  <span>{t('calculator.rate')}</span>
                  <span className="font-semibold text-slate-700">1 USD = {rate} {currency}</span>
                </div>
                <div className="flex justify-between border-t border-slate-200 pt-2.5 text-sm">
                  <span className="font-bold text-slate-800">{t('calculator.youReceive')}</span>
                  <span className="font-extrabold text-violet-600">
                    {receiveAmount.toLocaleString('fr-FR')} {currency}
                  </span>
                </div>
              </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-slate-500 max-w-lg mx-auto">
            Une plateforme conçue pour les freelancers, créateurs de contenus et e-commerçants africains.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            { icon: Zap, title: t('features.fast.title'), desc: t('features.fast.description') },
            { icon: ShieldCheck, title: t('features.secure.title'), desc: t('features.secure.description') },
            { icon: Percent, title: t('features.rates.title'), desc: t('features.rates.description') },
            { icon: MessageSquare, title: t('features.support.title'), desc: t('features.support.description') },
          ].map((feature) => (
            <Card key={feature.title} className="hover:border-violet-200 hover:shadow-lg transition-all duration-300 rounded-2xl bg-white border-slate-100 group">
              <CardContent className="p-6 space-y-4">
                <div className="w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center group-hover:bg-violet-600 group-hover:text-white transition-colors duration-300">
                  <feature.icon className="w-5 h-5" />
                </div>
                <h4 className="text-sm font-bold text-slate-950">{feature.title}</h4>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-6 gap-4">
          {countries.map((c) => (
            <div 
              key={c.name}
              className="flex items-center gap-3 p-3 border border-slate-100 rounded-2xl bg-white hover:border-slate-200 hover:bg-slate-50 transition-all duration-200"
            >
