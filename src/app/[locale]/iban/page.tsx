...
export default function IBANPage() {
  ...
  const handleCreateIBAN = async () => {
    ...
    try {
      ...
      setStripeDetails({
        email: 'Votre email PayMaestro',
        name: 'Votre nom complet',
        country: 'Pays associé au compte',
  ...
  const handleConfirmCreateIBAN = async () => {
    setCreating(true);
    try {
      const d = await api.stripe.createIBAN();
      setIban(d.iban);
      ...
      {showConfirm && stripeDetails && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl text-center">
            ...
            <h3 className="font-bold text-lg">Confirmer la création de l'IBAN</h3>
            <p className="text-sm text-gray-500 mb-4">
              Vérifiez les informations avant de générer votre IBAN européen :
            </p>
            
            <div className="bg-blue-50 rounded-xl p-4 text-left space-y-2 text-sm mb-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-800 dark:text-white">{stripeDetails.country}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Frais de réception :</span>
                <span className="text-green-600 font-bold">{stripeDetails.fees}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 dark:text-slate-400">Disponibilité :</span>
                <span className="text-green-600 font-semibold">{stripeDetails.estimatedTime}</span>
              </div>
            </div>
