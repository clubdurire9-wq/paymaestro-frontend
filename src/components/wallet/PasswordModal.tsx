...
export function PasswordModal({ onVerify, onClose }: PasswordModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(e.message || tErrors('invalidPassword'));
    }
    setLoading(false);
  };
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-sm w-full shadow-2xl dark:shadow-black/50">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">{t('passwordTitle')}</h3>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
            <X className="w-5 h-5" />
          </button>
        </div>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{t('passwordDesc')}</p>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800">{t('passwordCancel')}</button>
