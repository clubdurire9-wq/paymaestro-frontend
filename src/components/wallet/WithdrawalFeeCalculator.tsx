...
export function WithdrawalFeeCalculator({ usdBalance, onRefreshBalance }: WithdrawalFeeCalculatorProps) {
  ...
  const needsFx = channel === 'bank' && bankCurrency !== 'USD';
  const fxFee = needsFx ? roundCents(usdAmount * FX_FEE_PCT / 100) : 0;
  const totalFees = roundCents(processingFee + fxFee);
  const netUsd = roundCents(usdAmount - totalFees);

  const destCurrency = channel === 'mobile_money' ? mobileCountry.code : bankCurrency;
  ...
  ], [processingFee, fxFee, totalFees, netUsd, processingFeePct]);

  const handleConfirm = async () => {
    setLoading(true);
    try {
      const calls: Record<Channel, () => Promise<any>> = {
        mobile_money: () => api.wallet.withdrawMobile({
          amountUSD: usdAmount,
          currencyCode: destCurrency,
          phoneNumber: '',
          exchangeRate: destRate,
        }),
        bank: () => api.wallet.withdrawToWallet({
          amountUSD: usdAmount,
      ...
      setResultData({
        ...
        message: `Votre retrait de ${usdAmount.toFixed(2)} USD via ${channelConfig.label} a été pris en compte. Montant net : ${destAmount.toFixed(2)} ${destCurrency}.`,
      });
            ...
            <div>
              ...
              >
                ...
                {rates.filter(r => r.code !== 'USD').map(r => (
                  <option key={r.code} value={r.code}>{r.code} - {r.name} ({r.symbol})</option>
                ))}
              </select>
              {bankCurrency !== 'USD' ? (
                <p className="text-xs text-amber-600 dark:text-amber-400 mt-1 flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3" />
                  Frais de change de {FX_FEE_PCT}% appliqués — taux 1 USD = {rates.find(r => r.code === bankCurrency)?.rate || '?'} {bankCurrency}
                </p>
              ) : (
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Aucun frais de change</p>
              )}
            </div>
          )}
